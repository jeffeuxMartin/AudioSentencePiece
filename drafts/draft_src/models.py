...
class SpeechToSemantics(nn.Module):
    ...
    def generate(self,
        ...
        # === decoder pass === #
        # breakpoint()
        # B = self.representation_decoder.config.bos_token_id
...
class PLSpeechToSemantics(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if batch_idx % 10 == 10 - 1 or True or "$$$$$$$$$":  # TODO: detect log step?
            ...
            if self.wandb_logger is not None:
                self.wandb_logger.log_text(
                    key=f"Prediction v.s Ground truth (train)",
    ...
    def _step_unified(self, batch, batch_idx, mode="train"):
        # outputs, ZF = self(**batch)
        outputs = self(**batch)
        assert self.training == (mode == "train")
        if batch_idx % 10 == 0 or True or "$$$$$$$$$":
            # print(f"{batch_idx = }")
            pass
        if mode != "train" and (batch_idx == 10 - 1 or True or "$$$$$$$$$"):  # FIXME: 長度問題？ input 有沒有餵錯？
        ...
        self.log(f"{mode}_loss", outputs.loss, batch_size=self.batch_size, prog_bar=True) 
        ...
        if batch_idx % 10 == 10 - 1 or True or "$$$$$$$$$":
            return dict(
                loss=outputs.loss,
                preds=predicted_texts,
                target=groundtruth_texts,
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx == 10 - 1 or True or "$$$$$$$$$":  # FIXME: 長度問題？ input 有沒有餵錯？
            real_outputs = self.generate(
                input_ids=batch["input_ids"],
                attention_mask=batch["attention_mask"],
                word_length_tensor=batch["word_length_tensor"],
        ...
        if batch_idx == 10 - 1 or True or "$$$$$$$$$":
            ...
            demonstration_df.columns = [
                "predicted", 
                "real",
                "ground truth"]
            if self.wandb_logger is not None:
                self.wandb_logger.log_text(
                    key=f"Prediction v.s Ground truth (valid)",
                    dataframe=demonstration_df,
                )
