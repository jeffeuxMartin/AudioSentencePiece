# ALLbatch=18 GPUType=3090 GPUCount=3 zsh ~/AudioWords/AudioSentencePiece/scripts/exp_queue/fullunit_CIF_AE.sh
# ALLbatch=9
# GPUType=2080Ti    # 9                      
# GPUCount=3
#!sh
...
` # python3 AudioSentencePiece/main.py ` \
  --run_name 'fullunit_CIF_AE' \
  --output_dir "$OUTPUTDIR_PREFIX/$(date +%Y%m%d_%H%M%S)" \
  ` # setups ` ` # ~~~ # ` \
  --task AE \
  --datapath data/LibriSpeechUnits ` # AE ` \
